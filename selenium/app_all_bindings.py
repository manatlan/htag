...
function testall( ) {
    ...
    def init(self):
        ...
        self<= Tag.u("exit",_onclick = lambda o: self.exit())
...
def tests(client:hclient.HClient):
    ...
    client.click('//i')                         # click "TEST ALL"
    client.wait(1)                              # wait all processing
    assert client.find('//i')[0].text=="OK"     # ensure tests OK

    client.click('//u[text()="exit"]')          # force EXIT
    return True
...
if __name__=="__main__":
    ...
    # hclient.test( App, "PyScript", tests) #NEED a "uv build" before !!!!
