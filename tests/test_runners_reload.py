...
@patch("htag.runner.Reloader.run_with_reloader")
...
def test_base_run_with_reloader(mock_sleep, mock_popen):
    ...
    """
    from htag.runner import Reloader
...
@patch("uvicorn.run")
@patch("htag.runner.Reloader.run_with_reloader")
def test_webapp_reload_master(mock_run_reloader, mock_uvicorn):
    """Test WebApp reload master process"""
    from htag import WebApp
    
    runner = WebApp(MyTestApp)
    runner.run(reload=True, open_browser=False)
...
@patch("uvicorn.run")
@patch("htag.runner.Reloader.run_with_reloader")
def test_webapp_reload_child(mock_run_reloader, mock_uvicorn):
    """Test WebApp reload child process"""
    from htag import WebApp
    os.environ["HTAG_RELOADER"] = "1"
